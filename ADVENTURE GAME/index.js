"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var inquirer_1 = require("inquirer");
var chalk_1 = require("chalk");
var Player = /** @class */ (function () {
    function Player(name) {
        this.name = name;
        this.fuel = 100;
    }
    Player.prototype.fuelDecrease = function () {
        this.fuel -= 25;
        if (this.fuel < 0) {
            this.fuel = 0;
        }
    };
    Player.prototype.fuelIncrease = function () {
        this.fuel = 100;
    };
    return Player;
}());
var Opponent = /** @class */ (function () {
    function Opponent(name) {
        this.name = name;
        this.fuel = 100;
    }
    Opponent.prototype.fuelDecrease = function () {
        this.fuel -= 25;
        if (this.fuel < 0) {
            this.fuel = 0;
        }
    };
    return Opponent;
}());
function startGame() {
    return __awaiter(this, void 0, void 0, function () {
        var player, opponent, p1, o1, ask, num;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, inquirer_1.default.prompt({
                        type: "input",
                        name: "name",
                        message: "Please Enter Your Name:"
                    })];
                case 1:
                    player = _a.sent();
                    return [4 /*yield*/, inquirer_1.default.prompt({
                            type: "list",
                            name: "select",
                            message: "Please Select Your Opponent:",
                            choices: ["Skeleton", "Zombie"]
                        })];
                case 2:
                    opponent = _a.sent();
                    p1 = new Player(player.name);
                    o1 = new Opponent(opponent.select);
                    _a.label = 3;
                case 3:
                    if (!(p1.fuel > 0 && o1.fuel > 0)) return [3 /*break*/, 5];
                    return [4 /*yield*/, inquirer_1.default.prompt({
                            type: "list",
                            message: "Select the option",
                            name: "option",
                            choices: ["Attack", "Drink Portion", "Run For Your Life..."],
                        })];
                case 4:
                    ask = _a.sent();
                    if (ask.option === "Attack") {
                        num = Math.floor(Math.random() * 2);
                        if (num === 0) {
                            o1.fuelDecrease();
                            console.log(chalk_1.default.bold.red("".concat(o1.name, " fuel is ").concat(o1.fuel)));
                            console.log(chalk_1.default.bold.green("".concat(p1.name, " fuel is ").concat(p1.fuel)));
                        }
                        else {
                            p1.fuelDecrease();
                            console.log(chalk_1.default.bold.red("".concat(p1.name, " fuel is ").concat(p1.fuel)));
                            console.log(chalk_1.default.bold.green("".concat(o1.name, " fuel is ").concat(o1.fuel)));
                        }
                        if (o1.fuel <= 0) {
                            console.log(chalk_1.default.bold.green.italic("You Win"));
                            return [3 /*break*/, 5];
                        }
                        else if (p1.fuel <= 0) {
                            console.log(chalk_1.default.bold.red.italic("You Loose, Better Luck Next Time"));
                            return [3 /*break*/, 5];
                        }
                    }
                    else if (ask.option === "Drink Portion") {
                        p1.fuelIncrease();
                        console.log(chalk_1.default.bold.italic.green("You Drink Health Portion. Your Fuel is ".concat(p1.fuel)));
                    }
                    else if (ask.option === "Run For Your Life...") {
                        console.log(chalk_1.default.bold.red.italic("You Ran Away, Better Luck Next Time"));
                        return [3 /*break*/, 5];
                    }
                    return [3 /*break*/, 3];
                case 5: return [2 /*return*/];
            }
        });
    });
}
startGame();
